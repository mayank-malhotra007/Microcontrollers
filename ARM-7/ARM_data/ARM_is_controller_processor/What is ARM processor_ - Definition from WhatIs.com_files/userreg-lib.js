...
function getQuestionErrorMessages(inputId, divId, row) {
    ...
    if (row.VALIDATION_MESSAGE_QUESTION_INVALID_EMAIL_DOMAIN != undefined && row.VALIDATION_MESSAGE_QUESTION_INVALID_INVALID_EMAIL_DOMAIN != "") {
        result += "<p id='" + inputId + ".invalid.domain' class='" + errorMessageClass + " hidden'>" + row.VALIDATION_MESSAGE_QUESTION_INVALID_EMAIL_DOMAIN + "</p>";
    }
...
var urValidation = {
    ...
    validate : function(validation, oform) {
        ...
        try {
            ...
            for (var i = 0; i < fields.length; i++) {
                ...
                if (!input || input.getAttribute("disabled")!=null || evaluatedRadioCheckboxStatus[input.name] || this.isFieldDisplayNone(input)) { continue; }
                ...
                if (!isError && input.type == "text") {
                    ...
                    if (!isError && (emailDomain = validation.emailDomain) && this.arrayContains(emailDomain.fields, field)) {
                        isError = (!this.validateEmailDomain(value.toLowerCase()))
                        if (isError) { this.setError(input, isError, "emailDomain"); }
                    }
    ...
    validateEmailDomain : function(email) {
    	var domains = [ /@gmail\./, /@yahoo\./, /@hotmail\./, /@msn\./, /@live\./, /@outlook\./, /@mailinator\./, /@mail\./, /@aol\./, /@comcast\./, /@verizon\./, /@earthlink\./, /@rediffmail\./, /@cox\./, /@gmx\./, /@ymail\./, /@qq\./, /@126\./, /@163\./, /@sbcglobal\./, /@rocketmail\./, /@indiatimes\./, /@sina\./, /@bellsouth\./, /@btinternet\./ ];

    	for (var i = 0; i < domains.length; i++) {
    	    if (domains[i].test(email)) { 
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            if (currentClass) {
            	currentClass = currentClass.replace("error-NoStyle", "")
                                           ...
                                           .replace((validation.emailDomain) ? validation.emailDomain.highlightClass || "" : "", "")
    ...
    setErrorMessage : function(fields, validation, oform) {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            if (validation.msgInline && lastErrorType && lastErrorType != errorType) {
                ...
                if (lastErrorType == "emailDomain") { errorP = document.getElementById(baseInputId + ".invalid.domain"); }
            ...
            if (isError) {
                ...
                if (validation.msgInline) {
                    ...
                    if (errorType == "emailDomain") { errorP = document.getElementById(baseInputId + ".invalid.domain"); }
    ...
    getValidationFields : function(v) {
        ...
        if (v) {
            ...
            if ((ed = v.emailDomain) && ed.fields) { f = f.concat(ed.fields); }
    ...
    isFieldDisplayNone : function(el) {
        do {
            if(el && (el.style.display == "none" || window.getComputedStyle(el).getPropertyValue("display") == "none")) {
                return true;
            }
        } while( el && (el = el.parentNode) && el.tagName != "HTML" );
        return false;
    },
        ...
        $.fn.formProgress = function(options) {
            ...
            function fProcess() {
                ...
                var filled = $(sel).filter(function() {
                                       ...
                                       return this.value!="";
                                    }).not(':checkbox, :radio, :hidden').length + $(sel + ':checked').filter(":visible").length;
                var newArr = {
                    filled : filled,
            ...
            function totalInputs() {
                var names = [];
                $(sel + ":radio").filter(":visible").each(function() {
                    var n = $(this).attr('name');
                    if ($.inArray(n, names) < 0) {
                        names.push(n);
                    }
                });
                return $(sel).not(":radio, :hidden").length + names.length;
            }
        };
    })(jQuery);
